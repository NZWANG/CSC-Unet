...
'''
    UNet 3+ with Deep Supervision without Classification-guided Module (CGM)
'''
...
class UNet3PlusDeepSup(nn.Module):
    def __init__(self, n_classes=1, in_channels=3):
        ...
        ## -------------Encoder--------------
        self.conv1 = VGGBlock(self.in_channels, filters[0], filters[0])
    ...
    def forward(self, inputs):
        ...
        d5 = self.outconv5(hd5)  # [1 1024 40 30] [1 12 40 30]
        d5 = self.upscore5(d5)  # [1 12 640 480]

        d4 = self.outconv4(hd4)  # [1 320 80 60] [1 12 80 60]
        d4 = self.upscore4(d4)  # [1 12 640 480]

        d3 = self.outconv3(hd3)  # [1 320 160 120] [1 12 160 120]
        d3 = self.upscore3(d3)  # [1 12 640 480]

        d2 = self.outconv2(hd2)  # [1 320 320 240] [1 12 320 240]
        d2 = self.upscore2(d2)  # [1  12 640 480]

        d1 = self.outconv1(hd1)  # [1 320 640 480] [1 12 640 480]
...
class CSCUNet3PlusDeepSup(nn.Module):
    def __init__(self, n_classes, unfolding, in_channels=3):

        self.unfolding = unfolding
        super(CSCUNet3PlusDeepSup, self).__init__()
        self.in_channels = in_channels

        filters = [64, 128, 256, 512, 1024]

        ## -------------Encoder--------------
        self.conv1 = CSCBlock(self.in_channels, filters[0], filters[0], unfolding)
        self.maxpool1 = nn.MaxPool2d(kernel_size=2)

        self.conv2 = CSCBlock(filters[0], filters[1], filters[1], unfolding)
        self.maxpool2 = nn.MaxPool2d(kernel_size=2)

        self.conv3 = CSCBlock(filters[1], filters[2], filters[2], unfolding)
        self.maxpool3 = nn.MaxPool2d(kernel_size=2)

        self.conv4 = CSCBlock(filters[2], filters[3], filters[3], unfolding)
        self.maxpool4 = nn.MaxPool2d(kernel_size=2)

        self.conv5 = CSCBlock(filters[3], filters[4], filters[4], unfolding)
